...
* **v2.0.16**：新增浏览器书签收藏管理系统、支持自定义默认主页设置、出厂默认主页重置为腾讯视频。
* **v2.0.15**：彻底修复聚合网站（如 bad.news）点击视频闪退、加固全屏/多窗口与 JS 播放器 Hook、补全屏幕旋转生命周期防护。
* **v2.0.14**：在线更新国内多源加速与 GitHub 直连保全兜底机制、手机端播放全链路屏幕常亮保活、顶部导航栏新增一键刷新按钮。
* **v2.0.13**：剧集全链路自动连播与一键切集、彻底修复「清空重探」在播放中实时恢复视频流、多播放器实例内存流持久追踪。
* **v2.0.12**：智能过滤片头广告视频流 (`ad.m3u8`/`guanggao`)、页面跳转自动清理旧残留、候选弹窗智能高亮【推荐 / 最新】及提供「清空重探」功能。
* **v2.0.11**：彻底根治嗅探 `.ts` 切片导致 M3U8 二进制污染及 Kodi 报错问题、过滤切片伪候选。
* **v2.0.10**：深度修复嗅探误报网页导致 Kodi 播放失败的问题（`Error creating demuxer`）、严格过滤非流媒体请求、强制补全 `#EXTM3U` 标头。
